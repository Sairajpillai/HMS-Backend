...
public class MedicineServiceImpl implements MedicineService{
    ...
    public List<MedicineDTO> saveAllMedicines(List<MedicineDTO> requestList) {
        // List<Medicine> medicineEntities = requestList.stream()
        //     .map(MedicineDTO::toEntity)
        //     .toList();

        // // 2. Save entities in DB
        // List<Medicine> savedEntities = medicineRepository.saveAll(medicineEntities);

        // // 3. Convert saved entities → DTOs
